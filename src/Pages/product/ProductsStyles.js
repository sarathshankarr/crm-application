...
const ProductsStyles = ({route}) => {
  ...
  const getBarcod = async styleId => {
    const apiUrl = `${global?.userData?.productURL}${API.GET_BARCODE}/${styleId}/${companyId}`;
    try {
      ...
      const pdfPath =
        Platform.OS === 'android'
          ? `${ReactNativeBlobUtil.fs.dirs.DownloadDir}/Barcode${styleId}.pdf`
          : `${ReactNativeBlobUtil.fs.dirs.DocumentDir}/Barcode${styleId}.pdf`;
  ...
  const renderItem = ({item}) => (
    <TouchableOpacity
      ...
      <TouchableOpacity onPress={() => getBarcod(item?.styleId)}>
        <Image
          style={styles.cell4}
