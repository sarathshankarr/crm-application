...
const PriceList = ({route}) => {
  ...
  const handleNextproductImage = () => {
    const priceListData = Object.entries(tabPriceData).map(([priceListName, sizes]) => {
      const priceList = priceListOptions.find(pl => pl.priceListName === priceListName);
  ...
  // FIXED: Always use tabPriceData if available, otherwise initialize with empty prices
  const currentPriceData = tabPriceData[selectedTab] || initialPriceData.map(item => ({
    ...item,
    dealerPrice: '',
    retailerPrice: '',
    mrp: '',
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{flexDirection: 'row', alignSelf: 'center'}}>
        <TouchableOpacity onPress={handleNextBasicInfo} style={styles.headbasicinfo}>
          <Text style={{color:'#000', fontWeight: 'bold'}}>Basic Info</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.headpricelist}>
          <Text style={{color:'#000', fontWeight: 'bold'}}>Price List</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={handleNextproductImage}
          style={[
            styles.headprductimage,
          ]}>
      ...
      <View style={styles.navTabs}>
        <TouchableOpacity style={[styles.navTab, styles.activeNavTab]}>
          <Text style={[styles.navTabText, styles.activeNavTabText]}>Price List</Text>
      ...
      <ScrollView style={styles.tableContainer}>
        {selectedScale && currentPriceData.length > 0 ? (
          <FlatList
            ...
            keyExtractor={(item, index) => `${selectedTab}-${item.size}-${index}`}
            scrollEnabled={false}
          />
        ) : (
